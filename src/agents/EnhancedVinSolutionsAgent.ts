...
import { EmailService, EmailConfig, TwoFactorCode } from '../services/EmailService';
...
export class EnhancedVinSolutionsAgent {
  ...
  constructor(emailConfig: EmailConfig) {
    ...
    this.logger = new Logger('EnhancedVinSolutionsAgent');
  ...
  async loginWithEmailAuth(credentials: VinSolutionsCredentials): Promise<boolean> {
    if (!this.page) throw new Error('Page not initialized');
    ...
    try {
      ...
      if (pageContent?.includes('Verify your identity') || pageContent?.includes('verification')) {
        ...
        try {
          ...
          await this.emailService.sendNotificationEmail(
            'VinSolutions Login Failed',
            `Failed to receive 2FA code via email: ${emailError.message}`,
            true
          );
          return false;
        }
      ...
      if (currentUrl.includes('dashboard') || finalPageContent?.includes('VinSolutions')) {
        ...
        await this.emailService.sendNotificationEmail(
          'VinSolutions Login Successful',
          `AI Agent successfully logged into VinSolutions at ${new Date().toISOString()}`
        );
  ...
  async extractLeadSourceROI(): Promise<ExtractionResult> {
    ...
    try {
      ...
      await this.emailService.sendNotificationEmail(
        `${reportName} Extraction Failed`,
        `Failed to extract ${reportName} report: ${error.message}`,
        true
      );
  ...
  async runFullExtractionWorkflow(credentials: VinSolutionsCredentials): Promise<ExtractionResult> {
    try {
      ...
      if (result.success) {
        await this.emailService.sendNotificationEmail(
          ...
          `Successfully extracted ${result.reportName} report and sent via email.`
        );
      }
      ...
      await this.emailService.sendNotificationEmail(
        ...
        `The complete extraction workflow failed: ${error.message}`,
        true
      );
