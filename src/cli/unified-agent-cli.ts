...
async function validateConfig(config: Partial<UnifiedTaskConfig>): Promise<UnifiedTaskConfig> {
  try {
    return configValidator.validateAndMerge(config);
  } catch (error) {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      const config = await configValidator.loadFromFile(configPath);
...
program
  ...
  .action(async (options) => {
    const { DebugMonitoringService } = await import('../services/DebugMonitoringService');
    ...
    if (options.list) {
      ...
      const exportPath = await debugService.exportSession(
        options.export,
...
async function executeNaturalLanguageTask(
  ...
  options?: { verbose?: boolean; exportDebug?: string }
) {
 ...
 */
async function executeTask(config: UnifiedTaskConfig) {
  // Validate credentials
  if (!config.authentication.username || !config.authentication.password) {
    console.error('❌ Missing credentials! Set COX_USERNAME and COX_PASSWORD environment variables.');
    console.log('\nExample .env file:');
