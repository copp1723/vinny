...
export class SessionPersistenceService {
  ...
  constructor(config?: Partial<SessionPersistenceConfig>) {
    this.logger = new Logger('SessionPersistence');
  ...
  async saveSession(
    ...
    try {
      ...
      this.logger.stepFailed('Save session', error as Error);
      return false;
    }
  }
  ...
  async restoreSession(
    context: BrowserContext,
    identifier: string
  ): Promise<{ restored: boolean; page?: Page }> {
    if (!this.config.enabled) {
    ...
    try {
      ...
      // Load session data
      const sessionData: StoredSession = await fs.readJson(sessionFile);
      ...
      this.logger.stepFailed('Restore session', error as Error);
   ...
   */
  private async performKeepAlive(page: Page): Promise<void> {
    try {
  ...
  async cleanupOldSessions(): Promise<void> {
    try {
      ...
      for (const file of files) {
        ...
        try {
          const sessionData: StoredSession = await fs.readJson(filePath);
   ...
   */
  private getSessionFilePath(identifier: string): string {
  ...
  getSessionAge(identifier: string): string {
    try {
      const sessionFile = this.getSessionFilePath(identifier);
      const sessionData: StoredSession = fs.readJsonSync(sessionFile);
