...
const ReportExtractor_1 = require("../services/ReportExtractor");
const FileManager_1 = require("../utils/FileManager");
// Load environment variables
dotenv_1.default.config({ path: '.env.dashboard' });
...
app.use('/downloads', express_1.default.static(path_1.default.join(process.cwd(), 'downloads')));
// API endpoints
...
app.get('/api/download/:filename', (req, res) => {
    ...
    const filepath = path_1.default.join(process.cwd(), 'downloads', filename);
...
async function executeJob(job) {
    ...
    try {
        ...
        extractor = new ReportExtractor_1.ReportExtractor(config);
        await extractor.initialize();
        // Prepare report request
        const reportRequest = {
            reportName: job.reportName || `Report #${job.reportIndex}`,
            reportType: 'custom',
            outputFormat: (job.format || 'xlsx').toLowerCase(),
        ...
        if (result.success && result.filePath) {
            ...
            const filename = path_1.default.basename(result.filePath);
