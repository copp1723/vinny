...
exports.DemoVinSolutionsAgent = void 0;
...
const axios_1 = __importDefault(require("axios")); // Added axios for HTTP requests
class DemoVinSolutionsAgent {
    config;
    logger;
    mailgunService;
    openRouterService;
    ...
    constructor(config) {
        this.config = config;
        this.logger = new Logger_1.Logger('DemoVinSolutionsAgent');
        // Initialize services
    ...
    async extractLeadSourceROI() {
        ...
        try {
            ...
            await this.mailgunService.sendNotificationEmail('VinSolutions Extraction Started (Demo)', 'The AI agent has begun extracting the Lead Source ROI report from VinSolutions. This is a demonstration of the complete workflow including AI-powered automation and professional email delivery.', this.config.reportRecipients);
            // Launch browser
            ...
            if (has2FA) {
                ...
                if (isCodeEntryScreen || await this.isCodeEntryScreenVisible()) {
                    ...
                    if (!this.config.webhookUrl) {
                        ...
                        await this.mailgunService.sendNotificationEmail('Warning: Using Simulated 2FA Code', 'No webhook URL was configured. Using a simulated code (123456) instead of retrieving a real code from the email webhook.', this.config.reportRecipients);
                        ...
                        await this.mailgunService.sendNotificationEmail('2FA Code Retrieval', 'The agent is waiting for the 2FA code to arrive at the webhook. This typically takes 15-30 seconds after selecting the email option.', this.config.reportRecipients);
    ...
    async getCodeFromWebhook(screenshots) {
        if (!this.config.webhookUrl) {
            return null;
        }
        const maxRetries = 5;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const response = await axios_1.default.get(`${this.config.webhookUrl}/api/code/latest?platform=vinsolutions&minConfidence=0.7`, { timeout: 10000 });
    ...
    async enterTwoFactorCode(code, codeInputSelectors, screenshots) {
    ...
    async isCodeEntryScreenVisible() {
        const codeInputSelectors = [
            'input[placeholder*="code"]',
            'input[name*="code"]',
            'input[type="text"]',
    ...
    async extractReport(screenshots) {
        ...
        const downloadPath = path_1.default.join(this.config.downloadDir, `lead-source-roi-${Date.now()}.xlsx`);
...
exports.DemoVinSolutionsAgent = DemoVinSolutionsAgent;
//# sourceMappingURL=DemoVinSolutionsAgent.js.map